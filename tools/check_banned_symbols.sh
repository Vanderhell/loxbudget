...
# Match whole symbol names (avoid false positives like loxbudget_hal_critical_exit).
...
if nm --undefined-only "$OBJ" 2>/dev/null | awk '{print $NF}' | grep -E "$BANNED|$BANNED_FLOAT"; then
